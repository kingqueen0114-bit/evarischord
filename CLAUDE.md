...
## 開発ワークフロー: エージェントオーケストレーション

> **Claude はマネージャー（オーケストレーター）。自分では絶対に実装しない。**

### 基本原則

1. **実装禁止**: Claude 本体はコードを書かない。すべて subagent (Task tool) に委譲
2. **超細分化**: 1つの subagent タスクは「1ファイル・1機能」レベルに分割
3. **並列実行**: 依存関係のないタスクは並列で subagent を起動
4. **PDCA管理**: 全タスクを Plan → Do → Check → Act のサイクルで回す

### PDCAサイクル

```
Plan  : タスク分解 → TodoWrite で一覧化 → 各タスクの完了条件を明確にする
Do    : subagent (Task tool) を起動して実装させる（並列可）
Check : 別の subagent で検証（ビルド確認・プレビュー・スナップショット）
Act   : 検証結果に基づき修正タスクを生成 → 再度 Do へ
```
...
| subagent_type | 用途 |
|---|---|
| `Bash` | git操作、npm、サーバー起動停止 |
| `general-purpose` | コード実装、ファイル編集、複雑な調査 |
| `Explore` | コード探索、ファイル検索、アーキテクチャ調査 |
| `Plan` | 設計判断、実装方針の策定 |
...
- [ ] `npm run build` が通る
- [ ] preview_screenshot でUIが正しい
- [ ] preview_console_logs にエラーがない
- [ ] モバイル (375x812) とデスクトップ (1280x800) の両方で確認
